...
function checkInput() {
	...
	for(i=0 ; i<inputTextSplit.length ; i++) {
		...
		for(j=0;j<soundBox.length;j++) {
            innerFlag = ExistCheck(soundBox[j], textBuff, innerFlag);
        }
		if(!innerFlag) {
			judgeFlag = false;
			NGText += "\n"+textBuff;
...
function getFileName(phrase) {
    for (i = 0; i < soundBox.length; i++) {
        for (j = 0; j < soundBox[i].length; j++) {
            if (phrase == soundBox[i][j][1]) {
                return soundBox[i][j][0];
            }
        }
    }
}
